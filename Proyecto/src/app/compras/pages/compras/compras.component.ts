...
export class ComprasComponent implements OnInit {
  ...
  onHistorialCompras(product: Product): void {
    if (product.id) { // Verifica que el ID no sea undefined
  ...
  mostrarLotes(productId: string): void {
    this.loteService.getLotesByProduct(productId).subscribe({
      ...
      error: (error) => {
        ...
        this.showAlert('Error al obtener los lotes.', 'error'); // Muestra una alerta si hay error
      }
    });
  }
